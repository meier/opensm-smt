 ...
 *
 *        file: SMT_AnalysisManagerOrig.java
 *
 ...
 **********************************************************************/
@Deprecated
public class SMT_AnalysisManagerOrig implements SMT_AnalysisUpdater, Runnable, CommonLogger
{
  
  /** the one and only <code>SMT_AnalysisManager</code> Singleton **/
  private volatile static SMT_AnalysisManagerOrig gAnalysisMgr  = null;
  ...
  **/
   ...
   *
   ***********************************************************/
  private SMT_AnalysisManagerOrig()
  {
    super();
    initManager();
  }
     ...
     **************************************************************************/
     ...
     public static SMT_AnalysisManagerOrig getInstance()
     {
       synchronized( SMT_AnalysisManagerOrig.semaphore )
       {
         if ( gAnalysisMgr == null )
         {
           gAnalysisMgr = new SMT_AnalysisManagerOrig( );
         }
         return gAnalysisMgr;
       }
     }
     ...
     protected void createThread()
     {
       if (!Thread_Exists)
       {
         ...
         Update_Analysis_Thread.setName(this.getClass().getSimpleName() + " (" + update_counter + ")");
         
         logger.info("Creating the " + Update_Analysis_Thread.getName() + " Thread (" + update_counter + ")");
    ...
    **/
    ...
    private boolean startThread()
     {
       if(Thread_Exists || isBusy())
       {
         logger.warning("Thread: " + Update_Analysis_Thread.getName() + " is already running, unable to start another");
         logger.warning("Consider slowing playback of flight recorder data");
         return false;
       }
    ...
    public void destroy()
    {
     ...
     private boolean isBusy()
     {
       if(Update_Analysis_Thread == null)
         return false;
       logger.info("Checking the " + Update_Analysis_Thread.getName() + " Thread");
  ...
  **************************************************************************/
  ...
  public void osmServiceUpdate(OMS_Updater updater, OpenSmMonitorService osmService) throws Exception
  {
    ...
    //
    // consider doing in separate worker thread, otherwise you hold up the works
    
        // if the updater is an SMT_Updater, skip analysis, already done
        if(updater instanceof SMT_UpdateService)
        {
           logger.severe("The updater for the analysis is: " + ((SMT_UpdateService)updater).getName());
           // do analysis
           if(doAnalysis(((SMT_UpdateService)updater), osmService))
           {
             logger.severe("Updating listeners via starting the Analysis Listener update thread");
             startThread();
             logger.severe("Returned from starting the Analysis Listener update thread");
          }
           else
             logger.warning("Couldn't perform SMT analysis on osmService instance");
        }
        else
        {
  ...
  **/
   ...
   * @return
   ***********************************************************/
  private boolean doAnalysis(SMT_UpdateService smt_UpdateService, OpenSmMonitorService osmService)
  {
  ...
  public void run()
  {
    // as an optimization, consider using the alternate constructor
//    public OSM_FabricDeltaAnalyzer(OSM_FabricDelta delta, int numBins, LinkedHashMap <String, IB_Vertex> vertexMap, LinkedHashMap <String, IB_Edge> edgeMap, OSM_FabricAnalyzer fabAnalizer)
    ...
    // optimization:
    // don't analyze unless there are listeners
