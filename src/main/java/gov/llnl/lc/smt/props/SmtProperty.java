...
public enum SmtProperty implements Serializable, NetworkConstants
{
  ...
  SMT_DELIMITER(                8, "parsing delimiter",      "delimiter",   "delim",     "string",   "field separator (configured & current query option)"),
  SMT_VERBOSE(                  9, "parsing verbose",        "verbose",     "verbose",     "",   "provide additional information"),
  SMT_PRETTY(                  10, "parsing pretty",         "pretty",      "pretty",     "",   "format output for human consumption"),
  ...
  SMT_FILE_FILTER(            201,  "SmtFile.filter",         "filter",  "filter", "filter filename> <out filename", "writes a filtered file, using the supplied filter"),
  SMT_FILE_ANONYMIZE(         250,  "SmtFile.anonymize",      "anonymize",  "anon", "anonymizer filename> <out filename", "writes an anonymized file, using rules in the supplied anonymizer file"),
  SMT_ANONYMIZE_FILE(         251,  "SmtFile.info",           "anonymize",  "anon",  "filename", "a text file containing anonymization rules"),
  ...
  SMT_OMS_FILTERED_COLL_FILE( 201, "gov.llnl.lc.infiniband.opensm.plugin.data.OMS_FilteredCollection",    "readOMS_FilteredCollection","toInfo", "toTimeString", "the file of the OMS filtered collection cache"),
  ...
  SMT_OMS_ANONYMOUS_COLL_FILE(216, "gov.llnl.lc.infiniband.opensm.plugin.data.OMS_AnonymizedCollection",  "readOMS_AnonymizedCollection", "toInfo", "toTimeString", "the file of the Anonymized OMS collection cache"),
  ...
  SMT_GENERATE_XML(           301, "generateXMLconfig",                             "genXML",             "gX", "filename", "generate a topology (fabric config) file in XML using the fabrics current state."),
  SMT_GENERATE_JSON(          302, "generateJASONconfig",                           "genJSON",            "gJ", "filename", "generate a topology (fabric config) file in JSON using the fabrics current state."),
