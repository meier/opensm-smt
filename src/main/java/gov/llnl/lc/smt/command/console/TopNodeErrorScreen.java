 ...
 **********************************************************************/
public class TopNodeErrorScreen extends SmtConsoleScreen
{
  ...
  public boolean paintForeground() throws Exception
  {
    ...
    LinkedHashMap<String, IB_Vertex> atn = TA.getTopNodeChanges();
    ...
    printTitleString(String.format("Top %2d Active Error Nodes", num), titleRow);
    ...
    if((atn != null) && !(atn.isEmpty()))
    {
      printDataTimeStamp(TA.getAnalyzedTime(), titleRow, false);
       ...
       Map.Entry<String, IB_Vertex> me = atn.entrySet().iterator().next();
       ...
       for (Map.Entry<String, IB_Vertex> eMapEntry : atn.entrySet())
      {
         ...
         if(v != null)
         {
             Toolkit.printString(Integer.toString(v.getDepth()),         lvlCol+1,   listRow, FrgndTxtColor);
             Toolkit.printString(v.toVertexIdString(maxNameLen),         lvlCol+3, listRow, FrgndTxtColor);
             Toolkit.printString(v.toVertexErrorString(),                eCol, listRow, ErrorTxtColor);
         }
  ...
  public boolean paintBackground() throws Exception
  {
    ...
    printTitleString(String.format("Top %2d Active Error Nodes", TA.getNumberOfTop()), row++);
    ...
    Toolkit.printString("error:port list", eCol+12, row, BkgndTxtColor);
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    if(uService != null)
    {
      SmtConsoleScreen screen = new TopNodeErrorScreen();
      // start the service
  ...
  **/
  ...
  protected boolean updateForegroundData() throws Exception
  {
    ...
    if(cmd != null)
      TA.setMode(numTop, SmtProperty.SMT_NODE_ERRORS, false, cmd.getFilter());
    else
      TA.setMode(numTop, SmtProperty.SMT_NODE_ERRORS, false, new SmtFilter());
