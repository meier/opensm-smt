 ...
 *
 *        file: TopLinkTrafficScreen.java
 *
 ...
 **********************************************************************/
public class TopLinkTrafficScreen extends SmtConsoleScreen
{
  ...
  public boolean paintForeground() throws Exception
  {
    ...
    if(atl != null)
    {
       ...
       for (Map.Entry<String, IB_Edge> eMapEntry : atl.entrySet())
      {
        ...
        Toolkit.printString(Integer.toString(te.getDepth()),         lvlCol+1,   row, FrgndTxtColor);
        Toolkit.printString(te.toEdgeIdString(maxNameLen),           lvlCol+3, row, FrgndTxtColor);
        Toolkit.printString(te.toEdgeXmitString(),                   xCol+3, row, FrgndTxtColor);
        Toolkit.printString(te.toEdgeRcvString(),                    rCol+3, row, FrgndTxtColor);
        row++;
       }
    }
    else
  ...
  public boolean paintBackground() throws Exception
  {
    ...
    printTitleString(String.format("Top %2d Active Traffic Links", TA.getNumberOfTop()), row++);
    row++;
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    if(uService != null)
    {
      SmtConsoleScreen screen = new TopLinkTrafficScreen();
      // start the service
  ...
  **/
  ...
  protected boolean updateForegroundData() throws Exception
  {
    ...
    if(cmd != null)
      TA.setMode(numTop, SmtProperty.SMT_LINK_TRAFFIC, false, cmd.getFilter());
    else
      TA.setMode(numTop, SmtProperty.SMT_LINK_TRAFFIC, false, new SmtFilter());
