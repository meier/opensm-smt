...
/**********************************************************************
 * Describe purpose and responsibility of EventQuery
 * <p>
...
public enum EventQuery
{
   ...
   */
  public static final EnumSet<EventQuery> EVENT_ALL_QUERIES = EnumSet.allOf(EventQuery.class);
  
  
  private static final Map<Integer,EventQuery> lookup = new HashMap<Integer,EventQuery>();

  static 
  {
    for(EventQuery q : EVENT_ALL_QUERIES)
         lookup.put(q.getQNum(), q);
  }
  
  private static class AlphaCompare implements Comparator<EventQuery>
  {
    @Override
    public int compare(EventQuery q1, EventQuery q2) 
    {
  ...
  private EventQuery(int QNum, String Name, String ShortName, String Description)
  {
      this.QNum         = QNum;
      this.Name         = Name;
  ...
  public static EventQuery get(int qNum)
  { 
      return lookup.get(qNum); 
  }


  public static EventQuery getByName(String Name)
  {
  ...
  {
    return describeAllQueryTypes(EVENT_ALL_QUERIES);
  }

  public static String describeAllQueryTypes(EnumSet<EventQuery> eSet)
  {
    // put in alphabetical order
    TreeSet<EventQuery> ts = new TreeSet<EventQuery>(new AlphaCompare());
    for(EventQuery q : eSet)
    {
      if(q.equals(EVENT_LAST_QUERY))
        break;
      ts.add(q);
    }
    ...
    buff.append("usage: smt-event [-h=<host url>] [-pn=<port num>] -q <option>\n");
  ...
  private static int getMaxNameLength(EnumSet<EventQuery> eSet)
  {
    int len = 0;
    for(EventQuery q : eSet)
    {
