 ...
 *
 *        file: TopNodeTrafficScreen.java
 *
 ...
 **********************************************************************/
public class TopNodeTrafficScreen extends SmtConsoleScreen
{
  ...
  // I have a new Fabric, so calculate data
  {
    ...
    if(cmd != null)
      TA.setMode(numTop, SmtProperty.SMT_NODE_TRAFFIC, false, cmd.getFilter());
    else
      TA.setMode(numTop, SmtProperty.SMT_NODE_TRAFFIC, false, new SmtFilter());
  ...
  public boolean paintForeground() throws Exception
  {
    ...
    if(atn != null)
    {
       ...
       for (Map.Entry<String, IB_Vertex> eMapEntry : atn.entrySet())
      {
        ...
        Toolkit.printString(Integer.toString(tn.getDepth()),         lvlCol+1,   row, FrgndTxtColor);
        Toolkit.printString(tn.toTopPortIdString(maxNameLen),        lvlCol+3, row, FrgndTxtColor);
        Toolkit.printString(tn.toVertexXmitString(),                   xCol+3, row, FrgndTxtColor);
        Toolkit.printString(tn.toVertexRcvString(),                    rCol+3, row, FrgndTxtColor);
        row++;
       }
    }
    else
  ...
  public boolean paintBackground() throws Exception
  {
    ...
    printTitleString("Top " + Integer.toString(TA.getNumberOfTop()) +" Active Traffic Nodes", row++);
    row++;
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    if(uService != null)
    {
      SmtConsoleScreen screen = new TopNodeTrafficScreen();
      // start the service
