 ...
 **********************************************************************/
public class TopLinkErrorScreen extends SmtConsoleScreen
{
  ...
  public boolean paintForeground() throws Exception
  {
    ...
    LinkedHashMap<String, IB_Edge>        atl = TA.getTopLinkChanges();
    ...
    printTitleString(String.format("Top %2d Active Error Links", num), titleRow);
      ...
      printDataTimeStamp(TA.getAnalyzedTime(), titleRow, false);
       ...
       // fill up the area with link errors
         for (Map.Entry<String, IB_Edge> eMapEntry : atl.entrySet())
         {
           ...
           Toolkit.printString(Integer.toString(te.getDepth()),         lvlCol+1, listRow, FrgndTxtColor);
           Toolkit.printString(te.toEdgeIdString(maxNameLen),           lvlCol+3, listRow, FrgndTxtColor);
           Toolkit.printString(te.toShortErrorString(),                 eCol, listRow, ErrorTxtColor);
           listRow++;
          }
     }
    else
  ...
  public boolean paintBackground() throws Exception
  {
    ...
    printTitleString(String.format("Top %2d Active Error Links", TA.getNumberOfTop()), row++);
    ...
    Toolkit.printString("delta errs/period", ScreenCols-18, row, BkgndTxtColor);
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    if(uService != null)
    {
      SmtConsoleScreen screen = new TopLinkErrorScreen();
      // start the service
  ...
  **/
  ...
  protected boolean updateForegroundData() throws Exception
  {
    ...
    if(cmd != null)
      TA.setMode(numTop, SmtProperty.SMT_LINK_ERRORS, false, cmd.getFilter());
    else
      TA.setMode(numTop, SmtProperty.SMT_LINK_ERRORS, false, new SmtFilter());
