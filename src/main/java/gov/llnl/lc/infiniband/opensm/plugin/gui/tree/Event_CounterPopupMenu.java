...
public class Event_CounterPopupMenu extends JPopupMenu implements ActionListener, CommonLogger
{
  ...
  private OSM_EventStats Event_Stats;
  private OsmEvent EventType;
  ...
  **/
   ...
   * @param label
   ***********************************************************/
  public Event_CounterPopupMenu(String label)
  {
    this();
  ...
  **/
   ...
   * @param historySize2
   ***********************************************************/
  public Event_CounterPopupMenu(SubnetTreeModel model, OSM_EventStats event_Stats, OsmEvent selectedEvent, int historySize)
  {
  ...
  public void actionPerformed(ActionEvent e)
  {
    if(HistorySize < 2)
    {
      String msg = "Historical collection of Event Statistics unavailable, cannot create a plot.  Wait for more snapshots.";
      logger.severe(msg);
      MessageManager.getInstance().postMessage(new SmtMessage(SmtMessageType.SMT_MSG_SEVERE, msg));
      return;
    }
    
    if((Smodel != null) && (Event_Stats != null) && (HistorySize > 0))
    {
      ...
      JFrame plot = new XY_PlotFrame(type, Event_Stats, EventType);
