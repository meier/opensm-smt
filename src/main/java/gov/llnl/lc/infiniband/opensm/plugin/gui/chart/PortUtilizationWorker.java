 ...
 *
 *        file: PortUtilizationWorker.java
 *
...
/**********************************************************************
 * Used exclusively by the PortUtilizationPanel, which is in turn used
 ...
 * @author meier3
 * 
 * @version Oct 1, 2015 9:52:11 AM
 **********************************************************************/
public class PortUtilizationWorker extends SwingWorker<Void, Void> implements CommonLogger
{
  private PortUtilizationPanel PlotPanel;  // the parent or object that contains this worker
  private OMS_Collection History;
  private boolean UseService = false;        // if true, use the SmtService, if false, use default file

  public PortUtilizationWorker(PortUtilizationPanel portUtilizationPanel)
  {
    this(portUtilizationPanel, null);
  }

  public PortUtilizationWorker(PortUtilizationPanel portUtilizationPanel,boolean useService)
  {
    super();
    PlotPanel      = portUtilizationPanel;
    UseService     = useService;
    History        = null;
  }

  public PortUtilizationWorker(PortUtilizationPanel portUtilizationPanel, OMS_Collection history)
  {
    super();
    PlotPanel      = portUtilizationPanel;
    UseService     = false;
  ...
  protected Void doInBackground() throws Exception
  {
    ...
    Thread.currentThread().setName("PortUtilizationWorker");

//    JFreeChart Chart = PlotPanel.getUtilizationChart();

    logger.info("Worker Building PortUtilizationPlot");
    MessageManager.getInstance().postMessage(
        new SmtMessage(SmtMessageType.SMT_MSG_INFO, "Worker Building UtilizationPlot"));
    
    PortUtilizationDataSet pUtilDataSet = null;
    if(UseService)
    {
       SMT_UpdateService updateService = SMT_UpdateService.getInstance();
       History = updateService.getCollection();
       pUtilDataSet = new PortUtilizationDataSet(History);
   }
    ...
    {
        pUtilDataSet = new PortUtilizationDataSet("%h/scripts/OsmScripts/SmtScripts/sierra3H.his");
    }
//    logger.fine("Finished creating dataset");
//    logger.fine("Max X: " + pUtilDataSet.getMaximumXValue());
//    logger.fine("Max Y: " + pUtilDataSet.getMaximumYValue());
//    logger.fine("Max Z: " + pUtilDataSet.getMaximumZValue());
//    
    // if any of these "maximum" values are illegal, stop here and return null
    if(!pUtilDataSet.isValid())
    {
...
//    PlotPanel.setUtilizationDataSet(pUtilDataSet);
    ...
    Range fixedZRange = new Range(0, pUtilDataSet.getMaximumZValue()); // % Util
...
//    BufferedImage image = HeatMapUtilities.createHeatMapImage(pUtilDataSet, paintScale);
...
//    Chart.addSubtitle(psLegend);
  ...
  public void done()
  {
    ...
    logger.severe("Worker Done Building UtilizationPlot");
    ...
    MessageManager.getInstance().postMessage(
        new SmtMessage(SmtMessageType.SMT_MSG_INFO, "Worker Done Building UtilizationPlot"));
  }
