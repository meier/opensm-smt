...
public class IB_Decorator
{
  ...
  public static void setAllNameDecorators(HashMap<String, IB_Vertex> vMap, int nChars)
  {
    if((vMap != null) && (vMap.size() > 1))
    {
      ...
      {
        gov.llnl.lc.infiniband.opensm.plugin.graph.decorators.IB_Decorator td = new gov.llnl.lc.infiniband.opensm.plugin.graph.decorators.IB_Decorator(nVertexBins.getKey(k), k);
        System.err.println("The Vertices tagged (" + td.getName() + ") has " + vn.size() + " elements in it");
    ...
    if((vL != null) && (vL.size() > 0))
    {
      ...
      for(IB_Vertex tv: vL)
      {
        ...
        if(levelNumber == 1)
        {
          ...
          {
            tv.setDecorator(new gov.llnl.lc.infiniband.opensm.plugin.graph.decorators.IB_Decorator(LevelName, levelNumber));
            num++;
            if(tv.getEdgeMap().size() == 0)
              System.err.println("No edges for this vertex");
          }          
        }
        else
        {
      ...
      if(num > 0)
        System.err.println("There are " + num + " " + LevelName + " vertices");          
    }
    if((num <=0) && (levelNumber ==1))
    {
      System.err.println("Error, there must be at least one Level One vertex!");
      System.exit(-1);
    }
